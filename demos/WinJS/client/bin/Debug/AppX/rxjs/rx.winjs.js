...
(function (global) {
    ...
    root.Observable.fromGeolocator = function (statusChangedObserver) {
        return observableCreateWithDisposable(function (observer) {
            function handler(eventObject) {
                observer.onNext(eventObject);
            }

            function statusObserverHandler(eventObject) {
                statusChangedObserver.onNext(eventObject);
            }

            var geolocator = new Windows.Devices.Geolocation.Geolocator();
            geolocator.addEventListener('positionchanged', handler, false);

            if (statusChangedObserver) {
                geolocator.addEventListener('statuschanged', statusObserverHandler, false);
            }

            return disposableCreate(function () {
                geolocator.removeEventListener('positionchanged', handler, false);
                if (statusChangedObserver) {
                    geolocator.removeEventListener('statuschanged', handler, false);
                }
            });
        });
    };
